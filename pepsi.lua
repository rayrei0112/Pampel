local library = {
	Version = "0.33",
	WorkspaceName = "Zeta Hub Scripts",
	flags = {},
	signals = {},
	objects = {},
	elements = {},
				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Zeta Hub Scripts")
							if not fold then
								makefolder("./Zeta Hub Scripts")
							end
							fold = isfolder("./Zeta Hub Scripts/Themes")
							if not fold then
								makefolder("./Zeta Hub Scripts/Themes")
							end
							fold = isfolder("./Zeta Hub Scripts/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./Zeta Hub Scripts Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Zeta Hub Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Zeta Hub Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Zeta Hub Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Zeta Hub Scripts" then
		library.WorkspaceName = convertfilename(windowName, "Zeta Hub Scripts")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Zeta Hub Scripts") then
						makefolder("./Zeta Hub Scripts")
					end
					local common_string = "./Zeta Hub Scripts/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Zeta Hub Scripts/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			...
			Workspace = "Zeta Hub Scripts Themes",
			Flags = flags,
			Suffix = "Theme",
			Desginer = true
